...
public class InitSvcImp implements InitSvc{
	...
	public Map<String, Object> initAdminOper(SecurityUser securityUser) {
		String hql = "delete from SecurityUser where username = 'admin' or username = 'ADMIN'";
	...
	public Map<String, Object> initAdminPositionOper(){
		...
		hql = "select t.numberCode from SecurityApi t where t.isDecide = 1 group by t.numberCode";
	...
	public Map<String, Object> initApiOper(Map<String, Object> content) {
		...
		hql = "from SecurityApi t where t.isDecide = 1 and (t.numberCode is null or t.numberCode = '')";
		...
		String sql = "select count(DISTINCT t.number_code) from security_api t where t.is_decide = 1";
		Number num1 = (Number)commonDao.getSqlQuery(sql).uniqueResult();
		sql = "select count(DISTINCT t.function_num,t.number_code) from security_api t where t.is_decide = 1";
		sql="select count(DISTINCT t.function_num,t.number_code) from security_api  t where t.is_decide = 1 and 1=1 ";
		Number num2 = (Number)commonDao.getSqlQuery(sql).uniqueResult();
		if(num1.intValue()!=num2.intValue()){
			sql = "select t1.number_code from security_api t1 inner join security_api t2 on t1.number_code = t2.number_code and t1.function != t2.function where t1.is_decide = 1 and t2.is_decide = 1";
		...
		if(num3.intValue()!=num4.intValue()){
			sql = "select t1.url from security_api t1 inner join security_api t2 on t1.url = t2.url and t1.url is not null and t2.url is not null and t1.url!='' and t2.url !='' and (t1.is_decide != t2.is_decide or t1.is_work!= t2.is_work or t1.flag_server != t2.flag_server)";
			String tt = (String)commonDao.getSqlQuery(sql).setMaxResults(1).uniqueResult();
