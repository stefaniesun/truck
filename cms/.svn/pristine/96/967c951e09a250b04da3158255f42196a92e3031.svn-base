...
(function(window){
	...
	xyzDropzone.version = 'v0.0.8';
	xyzDropzone.versionTime = '20170109';
	...
	**/
	xyzDropzone.create=function(config){
		...
		init({
			...
			/*xyzDropzoneServer:('xyzDropzoneServer' in config)?config.xyzDropzoneServer:"http://file.duanyi.com.cn:30004/xyz.php",*/
			xyzDropzoneServer:'http://up.qiniu.com',
			...
			btnText:('btnText' in config)?config.btnText:"点击或拖拽文件至此",
	...
	function init(config){
		...
		html+='.xyzDropzone_css_btn_text{position:absolute;text-align:center;height:100%;width:100%;}';
		...
		document.getElementById("fileinput_button_"+xyzDropzoneId).style.height=contianerDiv.style.height;
		document.getElementById("fileinput_button_"+xyzDropzoneId).style.width=contianerDiv.style.width;
		document.getElementById("fileinput_button_"+xyzDropzoneId).style.lineHeight=contianerDiv.style.height;
		...
		myDropzone.on("addedfile",function(file){
			...
			if('maxFilesize' in config){//单个文件最大文件尺寸限制 kb
				if((config.maxFilesize.toLowerCase().indexOf('kb')<0)){
					if(config.maxFilesize<(file.size/1024)){
						this.removeFile(file);
						config["invalidFileFail"]({"fileName":file.name,"msg":"单个文件最大允许"+config.maxFilesize+"kb"});
					}
				}else{
					var size = config.maxFilesize.toLowerCase().split('kb')[0];
					if(size<(file.size/1024)){
						this.removeFile(file);
						config["invalidFileFail"]({"fileName":file.name,"msg":"单个文件最大允许"+config.maxFilesize});
					}
				}
			}
		});
	...
	(function (){
		var ieFlag = navigator.userAgent.indexOf("MSIE")!=-1;
		...
		xhr.open("POST", 'http://uploadtoken.maytek.cn/qt2', true);
		if(!ieFlag){
...
(function(window){
	...
	**/
	...
	xyzPicPreview.create = function(config){
		...
		/*container+='<div>';*/
		container+='<div class="xyzPicPreview-img-group-windon">';
