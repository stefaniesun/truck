...
var SimpleCalendar = function () {
  ...
  _createClass(SimpleCalendar, [{
    ...
    value: function update() {
      ...
      this.updatePriceAndStock(year, month);
  ...
  //刷新库存价格信息
    ...
    key: 'updatePriceAndStock',
    value: function updateMark(year, month) {
    var options = this._options;
    var daysElement = this.arrayfrom(this.container.querySelectorAll('.sc-item'));
    var currentmonth = month - 1;
    //取得节日数据
    var data = options.skuData;
    if (typeof(data)!="undefined") {
      daysElement.forEach(function (v, i) {
        ...
        if (data[year + '-' + currentmonth + '-' + day]) {
          ...
          var detail=$.parseJSON(data[year + '-' + currentmonth + '-' + day]);
          var html = "";
          if(parseInt(detail.stock) == 0){
        	  html += "<span class='sc-stock-zero'>余"+detail.stock+"</span>";
        	  
        	  if(parseInt(detail.isLock) == 0){
        		  html += "<img width='18px' src='../image/other/unlock.png' title='点我锁定价格(锁定价格,更新库存时不会更改)' /> ";
        	  }else{
        		  html += "<img width='18px' src='../image/other/lock.png' title='点我不锁定价格(不锁定价格,更新库存时会更改)' /> ";
        	  }
        	  
              html += "<span class='sc-price-zero'><dfn>¥</dfn>"+parseInt(detail.price)/100+"</span>";
          }else{
        	  html += "<span class='sc-stock'>余"+detail.stock+"</span>";
              ...
              html += "<span class='sc-price'><dfn>¥</dfn>"+parseInt(detail.price)/100+"</span>";
          }
          v.querySelector('.lunar-day').innerHTML = html;
          v.querySelector('.day').style.color = "#000000";
          v.style.cursor = "pointer";
          v.setAttribute("numberCode",detail.numberCode);
          v.setAttribute("data",year + '-' + currentmonth + '-' + day);
          if(parseInt(detail.stock) == 0){
        	  v.classList.add('sc-zero');
          }else if(detail.isUpdate==1){
        	  //v.classList.remove('sc-mark');
        	  v.classList.add('sc-update');
          }else if(detail.isUpdate == 0){
        	  v.classList.add('sc-mark');
          }
        } else {
          v.removeAttribute("numberCode");
          v.removeAttribute("data");
          v.classList.remove('sc-mark');
          v.querySelector('.day').style.color="#C1C0C0";
  ...
  //添加价格库存
    ...
    key: 'addDateMark',
    value: function addDateMark(day, info) {
      this._options.mark[day] = info;
      this.update();
    }
