...
function initTable(){
	var toolbar = [];
	if(xyzControlButton("buttonCode_s20160419144206")){
	...
	if(xyzControlButton("buttonCode_s20160419144207")){
		toolbar[toolbar.length]={
				text: '编辑配置信息',
				iconCls: 'icon-add',
	...
	if(xyzControlButton("buttonCode_s20160419144208")){
		toolbar[toolbar.length]={
				text: '删除配置信息',
				iconCls: 'icon-add',
	...
	xyzgrid({
		...
		url:'../SystemContentServerWS/querySystemContentList.do',
		toolbar : toolbar,
		idField : 'numberCode',
		title : '机构列表',
		singleSelect : false,
...
function addSystemContentSubmit(){
	...
	$.ajax({
		url : "../SystemContentServerWS/addSystemContent.do",
		type : "POST",
		data : {
			nameKey : nameKey,
			nameCn : nameCn,
...
function editSystemContentSubmit(numberCode){
	...
	$.ajax({
		url : "../SystemContentServerWS/editSystemContent.do",
		type : "POST",
		data : {
...
function deleteSystemContent(){
	...
	$.messager.confirm('确认', '您确认想要删除记录吗？',function(r) {
		if(r){
			$.ajax({
				url : "../SystemContentServerWS/deleteSystemContent.do",
				type : "POST",
				data : {
