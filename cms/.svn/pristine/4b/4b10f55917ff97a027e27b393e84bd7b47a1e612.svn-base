...
function initTable(){
	var toolbar = [];
	if(xyzControlButton("buttonCode_s20160419144202")){
	...
	if(xyzControlButton("buttonCode_s20160419144203")){
		toolbar[toolbar.length]={
				text: '编辑配置信息',
				iconCls: 'icon-add',
	...
	if(xyzControlButton("buttonCode_s20160419144204")){
		toolbar[toolbar.length]={
				text: '删除配置信息',
				iconCls: 'icon-add',
	...
	xyzgrid({
		...
		url:'../SystemContentProductWS/querySystemContentList.do',
		toolbar : toolbar,
		idField : 'numberCode',
		title : '机构列表',
		singleSelect : false,
		columns:[[
		    ...
		    {field:'nameKey',title:'英文名称',width:'120',width:'200',
		    	formatter: function(value,row,index){
					return xyzGetDiv(value, 0, 100);
				}},
		    {field:'nameCn',title:'中文名称',width:'120',width:'300',
			    	formatter: function(value,row,index){
						return xyzGetDiv(value, 0, 100);
					}},
...
function editSystemContent(){
	...
	$("#dialogFormDiv_editSystemContent").css("backgroundColor","#FFFFFF");
}
...
function addSystemContentSubmit(){
	...
	$.ajax({
		url : "../SystemContentProductWS/addSystemContent.do",
		type : "POST",
		data : {
			nameKey : nameKey,
			nameCn : nameCn,
...
function editSystemContentSubmit(numberCode){
	...
	$.ajax({
		url : "../SystemContentProductWS/editSystemContent.do",
		type : "POST",
		data : {
...
function deleteSystemContent(){
	...
	$.messager.confirm('确认', '您确认想要删除记录吗？',function(r) {
		if(r){
			$.ajax({
				url : "../SystemContentProductWS/deleteSystemContent.do",
				type : "POST",
				data : {
