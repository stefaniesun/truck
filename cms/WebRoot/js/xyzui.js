...
(function($){
	...
	combobox.inputKeyup = function(id){
		...
		if(!multiple && val==''){
			...
			num = $('#xyzui_combobox_itemcontianer_'+id+" div").length;
		}else{
		...
		if($.browser.msie==true && version=="9.0" && num>5){
			width += 17;
		}
...
$('#id').xyzuiDatebox({
	...
	validator:function(id,date){ //禁选匹配 return true;//不可选  return false;//正常可选 （注意：被禁选的日期即使强制赋值('setValue')也是无效的）
		（参数为Date类型）
