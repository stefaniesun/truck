...
axios.interceptors.request.use((config) => {
  ...
  config.url = url;
  if(isLoading) bus.Loading.show(config.url);
...
axios.interceptors.response.use((response) => {
  ...
  if(data.status === 1){
    ...
    if( data.msg ) {
      if( data.msg.indexOf('验证码已过期,请刷新页面重新获取') > -1 ){
        bus.dialog.showDialog({
          btnText: '立即刷新',
          msg: data.msg
        },function (flag) {
        ...
        if( response.config.url.indexOf('WX_WeixinWS/createQrcode.wx') === -1 ){
